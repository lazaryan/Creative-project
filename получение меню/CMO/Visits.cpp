...
///////////////////////////////////////////////////////////////////////
			...
			ПОЛУЧЕНИЕ ВРЕМЕНИ ПРИБЫВАНИЯ В КАФЕ
...
bool Visits::RecordVisits() 
{
	...
	try
	{
		fil = gcnew StreamWriter("Text.txt", true);
	}
	catch (...)
	{
		chek = false;
	...
	{
		fil->Write("{0};", Name);
		fil->WriteLine("{0}:{1}:{2}", TimeStart->house, TimeSta
			rt->minutes, TimeStart->seconds);
	}
	fil->Close();
	return chek;
}